		...
		(async () => {
			...
			if (status === 'granted') {
				const { data } = await Contacts.getContactsAsync({
					// Solicita o primeiro nome para exibir apenas o primeiro nome do contato
					fields: [Contacts.Fields.FirstName, Contacts.Fields.PhoneNumbers, Contacts.Fields.Emails],
				});
				if (data.length > 0) {
					// Filtrar apenas contatos cujo nome começa com a letra 'C' (case-insensitive)
					const filtered = data.filter((c) => {
						if (!c.name) return false;
						return c.name.trim().charAt(0).toLowerCase() === 'c';
					});
